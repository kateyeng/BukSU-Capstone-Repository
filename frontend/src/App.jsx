...
function UseOauthNotice() {
  ...
  useEffect(() => {
    ...
    if (shouldVerify) {
      toast("Your account is being verified. Please check your email.");
      console.log("[OAuth] pending verification. isNew:", isNew, "role:", role);
    } else {
...
const isPrivilegedUser = (user) => !!user && user.role !== "guest";
...
function StudentHome({ onLogout }) {
  const nav = useNavigate();
  return <StudentDashboard onLogout={onLogout} onNavigate={(dest, id) => mapStudentNav(nav, dest, id)} />;
}

function StudentBrowsePage({ onLogout }) {
  const nav = useNavigate();
  return <StudentBrowse onLogout={onLogout} onNavigate={(dest, id) => mapStudentNav(nav, dest, id)} />;
}

function StudentUploadsPage({ onLogout }) {
  const nav = useNavigate();
  return <StudentUploads onLogout={onLogout} onNavigate={(dest, id) => mapStudentNav(nav, dest, id)} />;
}

function StudentDetailsPage({ onLogout }) {
  const { id } = useParams();
  const nav = useNavigate();
  return <StudentDetails id={id} onLogout={onLogout} onNavigate={(dest) => mapStudentNav(nav, dest, id)} />;
}

function StudentAboutPage({ onLogout }) {
  const nav = useNavigate();
  return <StudentAbout onLogout={onLogout} onNavigate={(dest, id) => mapStudentNav(nav, dest, id)} />;
}

function StudentContactPage({ onLogout }) {
  const nav = useNavigate();
  return <StudentContact onLogout={onLogout} onNavigate={(dest, id) => mapStudentNav(nav, dest, id)} />;
}

function StudentProfilePage({ onLogout }) {
  return <StudentProfile onLogout={onLogout} />;
}
...
export default function App() {
  ...
  const logout = async () => {
    try {
      ...
      if (res.status === 200 || res.status === 401) toast.success("Logged out successfully");
      else {
        const err = await res.json().catch(() => null);
        toast.error(err?.message || "Logout failed on server.");
      }
    } catch {
  ...
  return (
    <>
      ...
      <Routes>
        ...
        <Route
          path="/browse"
          element={user && user.role === "student" ? <Navigate to="/student/browse" replace /> : <PublicBrowsePage />}
        />
        ...
        <Route path="/forgot-password" element={<ForgotPage setEmail={setEmailForReset} />} />
