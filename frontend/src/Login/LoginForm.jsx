...
export default function LoginForm({ onSwitch, onForgot, onSuccess }) {
  ...
  // Cleanup on unmount to stop widget timers
  useEffect(() => {
    return () => {
  ...
  try {
    ...
    alert(data.message);
    onSuccess?.(data.user);
    // optional: on success you usually DON'T need to reset captcha
  } catch (error) {
    alert(error?.response?.data?.message || "Login failed");

    // 🔁 Re-enable reCAPTCHA for another try
    try {
  ...
  return (
    <form className="form" onSubmit={handleSubmit}>
      ...
      {/* Login Button */}
      <button className="btn-primary" type="submit" disabled={loading || !captcha}>
