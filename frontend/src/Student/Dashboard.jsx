  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setStatsErr("");
        // ?mine=1 scopes totals + pending to the logged-in teacher
        const res = await api.get("/api/publicProjects/stats?mine=1", { signal: ac.signal });
  ...
  <a href="#" onClick={go("profile")}>Profile</a>
      ...
      <main className="hero">
        <div className="hero-content">
          ...
          <p>
            Discover, explore, and share academic excellence. A centralized platform for thesis and capstone
            projects at Bukidnon State University College of Technology.
          </p>
          <div className="hero-buttons">
            <button className="browse-btn" onClick={go("browse")}>Browse Projects</button>
            <button className="submit-btn" onClick={go("upload")}>Submit Thesis</button>
          </div>
        </div>
      </main>

      {/* Teacher Stats including "Pending Uploads" scoped to this teacher */}
      <section className="stats-teacher">
        {[
          { label: "Total Projects", value: stats.total, sub: statsErr ? "Error" : "Published Projects", icon: "book" },
          { label: "Latest Uploads", value: stats.latestUploads, sub: statsErr ? "Error" : "Past 7 days projects", icon: "upload" },
          { label: "My Pending Uploads", value: stats.pendingUploads, sub: statsErr ? "Error" : "Pending Approval", icon: "eye" },
          { label: "My Bookmarks", value: bookmarks.isLoading ? null : bookmarksCount, sub: "Saved documents", icon: "star" },
        ].map((s) => (
          <div key={s.label} className="stat-card">
            <div className="stat-left">
              ...
              <div className="stat-sub">
                {s.value == null && !statsErr ? (loadingStats ? "Loading..." : "—") : s.sub}
              </div>
            </div>
      ...
      {/* My Bookmarks (replaces Featured) */}
      <section className="featured">
        ...
        <p className="featured-subtitle text-center">
          Quick access to papers you saved. You can add/remove bookmarks from any project’s details page.
        </p>

        {bookmarks.isLoading && <p className="text-center">Loading bookmarks...</p>}
        {bookmarks.isError && (
        ...
        {!bookmarks.isLoading && !bookmarks.isError && (bookmarks.data || []).length === 0 && (
